  ...
  >
    <div class="flex flex-col items-start py-2 pr-4">
      ...
      >
        ...
        <p v-if="show_menu || LeftNavbar_open" class="font-medium text-sm pr-3">
          Compose
        </p>
      </div>
      ...
      <div
        ...
        :key="item.text"
        @click="updateFilter(item.text)"
      >
        <RouterLink :to="`/${item.text}`">
          <div
            class="flex items-center justify-start gap-5 px-4 pl-6 hover:cursor-pointer hover:bg-gray-200 w-full py-1.5 rounded-r-full"
            :class="clicked === item.text ? 'bg-[#D3E3FD]' : ''"
          >
            <Icon
              :icon="clicked === item.text ? item.icon2 : item.icon1"
              style="color: black; font-size: 1.25rem"
            />
            <p v-if="show_menu || LeftNavbar_open" class="text-sm">
              {{ item.text }}
            </p>
          </div>
        </RouterLink>
...
const { LeftNavbar_open, Compose_open } = storeToRefs(gmailstore);

const updateFilter = (filter) => {
  gmailstore.currentFilter = filter;
  clicked.value = filter;
  console.log("Current filter is ", gmailstore.currentFilter);
};
...
const menuItems = [
  ...
  {
    icon1: "mdi:send-variant-outline",
    icon2: "mdi:send-variant",
    text: "Sent",
  },
  {
    icon1: "mdi:file-document-outline",
    icon2: "mdi:file-document",
    text: "Drafts",
  },
