<template>
  ...
  <!-- Top Filters -->
  <div
    class="flex items-center justify-between px-2.5 pt-3 pb-3 sticky top-0 z-10 w-full bg-white"
  ...
  >
    ...
    <div class="flex items-center justify-between">
      <div class="flex items-center justify-center relative">
        <div
          class="flex p-2.5 px-2 mr-1 rounded-md gap-1"
          :class="
            selectAll || showSelectMenu ? 'bg-gray-200' : 'hover:bg-gray-100'
          "
        >
          ...
          >
            ...
            >
              <div
                ...
                class="hover:bg-gray-200 text-gray-600 hover:text-gray-800 cursor-pointer w-full pl-4 pr-12 py-2"
              >
                {{ x }}
              </div>
            </div>
      ...
      <div
        @click="gmailStore.toggleAllRead"
        class="flex gap-1 items-center justify-center p-3 rounded-full hover:bg-gray-100"
      >
        <Icon icon="mdi:more-vert" />
      </div>
      <div
        v-show="gmailStore.SelectedMessages.length"
        @click="gmailStore.deleteSelectedMessages"
        class="flex gap-1 items-center justify-center p-3 rounded-full hover:bg-gray-100"
      >
        <Icon
          icon="mdi:trash-can-outline"
          style="color: #5f6368; font-size: 1.12rem"
        />
      </div>
    </div>
  ...
  <div class="w-full mt-12 text-sm">
    ...
    <message
      v-if="loaded"
      v-for="xmessage in filteredList"
...
const { MessageList, DraftList, loaded } = storeToRefs(gmailStore);
const { getMailsByRecipient, getDraftsBySender } = gmailStore;
...
console.log(gmailStore.currentFilter);

// for first load ...
watch(
  () => gmailStore.loaded,
  (loaded) => {
    if (loaded) filteredList.value = MessageList.value;
    console.log("loaded is now :", loaded);
  }
);

// for later filters
watch(
  () => gmailStore.currentFilter,
  (filter) => {
    if (filter === "Drafts") filteredList.value = DraftList.value;
    else {
      filteredList.value = MessageList.value.filter((message) => {
        if (filter === "Inbox") return (filteredList.value = MessageList.value);
        if (filter === "Starred") return message.message.Status.Star;
        else if (filter === "Snoozed") return message.message.Status.Snooze;
        else if (filter === "Sent")
          return message.message.Sender === "Mohammed Adel"; // hard coded : need to be replaced later
      });
    }
  }
);
