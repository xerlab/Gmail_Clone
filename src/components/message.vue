  ...
  >
    <div class="flex items-center justify-center">
      <div
        ...
        class="flex items-center justify-center p-2.5 rounded-full"
        :class="selected ? 'hover:bg-blue-200' : 'hover:bg-gray-100'"
      >
        <Icon
          :icon="
            selected ? 'mdi:checkbox-outline' : 'mdi:check-box-outline-blank'
          "
      ...
      <div
        @click="toggleState('Star', Star)"
        class="flex items-center justify-center p-2.5 rounded-full -ml-2.5"
        :class="selected ? 'hover:bg-blue-200' : 'hover:bg-gray-100'"
      >
        <Icon
    ...
    <p
      class="truncate w-32 pr-4 flex-shrink-0"
      :class="Read ? 'font-thin text-gray-600' : 'font-medium'"
    >
      {{ Sender }}
    </p>
    <p
      @click="router.push({ path: `/messageid/${id}` })"
      class="truncate font-light flex-grow text-gray-500"
    >
      <span
    ...
    <div class="justify-end mr-1 ml-3" :class="hovered ? 'w-48' : 'min-w-14'">
      <div v-if="hovered" class="flex items-center justify-end gap-1">
        <div
          @click="toggleState('Archive', Archive)"
          class="flex items-center justify-center p-2.5 rounded-full"
          :class="selected ? 'hover:bg-blue-200' : 'hover:bg-gray-100'"
        >
          <Icon
        ...
        <div
          @click="gmailStore.deleteMessage(message_id)"
          class="flex items-center justify-center p-2.5 rounded-full"
          :class="selected ? 'hover:bg-blue-200' : 'hover:bg-gray-100'"
        >
          <Icon
        ...
        <div
          @click="toggleState('Read', Read)"
          class="flex items-center justify-center p-2.5 rounded-full"
          :class="selected ? 'hover:bg-blue-200' : 'hover:bg-gray-100'"
        >
          <Icon
            :icon="Read ? 'mdi:email-check' : 'mdi:email-open-outline'"
            style="color: #5f6368; font-size: 1.25rem"
          />
        </div>
        <div
          @click="toggleState('Snooze', Snooze)"
          class="flex items-center justify-center p-2.5 rounded-full"
          :class="selected ? 'hover:bg-blue-200' : 'hover:bg-gray-100'"
        >
          <Icon
            :icon="Snooze ? 'mdi:clock-check' : 'mdi:clock-outline'"
            style="color: #5f6368; font-size: 1.25rem"
          />
        </div>
      </div>
