...
export const useGmailStore = defineStore('Gmail',() => {
  ...
  function removefromSelected(message_id){
    SelectedMessages.value = SelectedMessages.value.filter(current_id => current_id !== message_id.value)
  ...
  function customSelection(selection){ 
    ...
    console.log("selection",selection, "Selection Name is :",selectionName[selection])
    if(selection === 'Read' || selection === 'UnRead'){
      if(selection === 'Read') SelectedMessages.value = MessageList.value.filter((message)=>  message.message.Read === true ).map((message) => message.id)
      else SelectedMessages.value = MessageList.value.filter((message)=>  message.message.Read !== true ).map((message) => message.id)
    }else if(selection === 'All' || selection === 'None'){ 
      if(selection === 'All') selectAllMessages() 
      else unselectAllMessages()
    }
    else{
      SelectedMessages.value = MessageList.value.filter((message)=>  message.message.Status[selectionName[selection]] === true ).map((message) => message.id)
    }
  ...
  function toggleAllRead(){
    if(selectAllRead.value !== ''){
      ...
      selectAllRead.value.forEach(async(message_id)=> {
        try { await updateDoc(doc(collection(db, "messages"), message_id), { Read: true })} 
        catch (error) { console.error("Error updating document:", error); } 
      })
    }
  }
  
  return {LeftNavbar_open,Compose_open,MessageList,loaded,SelectedMessages,selectAllRead,getMailsByRecipient,DateFormatter,deleteMessage,addtoSelected,removefromSelected,selectAllMessages,unselectAllMessages,customSelection,deleteSelectedMessages,toggleAllRead}        
})
